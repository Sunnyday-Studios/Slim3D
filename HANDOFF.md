...
| Floor surfaces | (this session) | **Rendered tabletop** the slime rests on (it used to float on nothing). `fluid.wgsl` raycasts the y=0 floor plane and samples a selectable CC0 image (Marble/Wood/Granite/Concrete/Terrazzo/Cream, or None) with a **dark backdrop** where the ray misses (contrast) + distance fade. `fluidRender.setSurface(url)` streams the image into one 1024² texture (@binding 6, repeat sampler @8); scene uniform @7 = `{bg, surfaceScale, surfaceEnabled}`. **Slime Lab "Surface" selector** (`controls.ts`). Textures in `public/textures/` (ambientCG CC0, see SOURCE.txt). Verified in the harness; **needs a device pass** for feel/scale. |
