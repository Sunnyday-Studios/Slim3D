...
fn g2p(@builtin(global_invocation_id) id: vec3<u32>) {
    if (id.x < arrayLength(&particles)) {
        ...
        // polar_R's det<1e-6 guard does NOT catch NaN (NaN comparisons are false),
        // so we must scrub it HERE before it cascades. det3 is inlined to avoid a
        // cross-file dependency (g2p does not import the p2g_2 helpers).
        let dF: f32 =
